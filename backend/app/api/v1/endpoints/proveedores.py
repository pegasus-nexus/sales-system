...
async def migrate_proveedores(current_user: User = Depends(get_current_active_user)):
    ...
    # Encontrar nombres únicos
    pipeline = [
        {"$match": {"tenant_id": tenant_id, "proveedor": {"$ne": None, "$ne": ""}}},
        {"$group": {"_id": "$proveedor"}}
    ]
    ...
    for nombre in nombres_proveedores:
        ...
        existe = await Proveedor.find_one(Proveedor.tenant_id == tenant_id, Proveedor.nombre == nombre)
        if not existe:
            nuevo = Proveedor(
                tenant_id=tenant_id,
                nombre=nombre.strip(),
                notas="Migrado desde producto"
            )
            await nuevo.insert()
            creados += 1
