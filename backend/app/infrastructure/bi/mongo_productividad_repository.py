...
class MongoProductividadRepository:
    """
    Repositorio de lectura limpia para el Módulo de BI de Productividad de Cajeros y Auditoría Operacional.
    Lee directamente las colecciones operacionales MongoDB: sales, audit_logs, users.
    """

    async def get_raw_sales_for_cashiers(
        self,
        user: User,
    ...
    async def get_audit_logs_summary(self, tenant_id: str) -> List[Dict[str, Any]]:
        db = await get_raw_db()
        ...
        cursor = db.audit_logs.find(filter_query)
        docs = await cursor.to_list(length=None)
        res = []
        for d in docs:
            res.append({
                "_id": str(d["_id"]),
                "action": str(d.get("action") or "OTRO"),
                "username": str(d.get("username") or "Sistema"),
                "entity": str(d.get("entity") or "GENERAL")
            })
        return res
