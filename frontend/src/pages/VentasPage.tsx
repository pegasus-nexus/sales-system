...
import { getSales, anularSale, getSucursales, toggleFacturaEmitida, checkPosibleDuplicado, getSesionesAbiertas, updateSaleDate, type MotivoAnulacion } from '../api/api';
...
export default function VentasPage() {
    ...
    const [updateDateVenta, setUpdateDateVenta] = useState<Sale | null>(null);
    const [nuevaFecha, setNuevaFecha] = useState('');
    
    const updateDateMut = useMutation({
        mutationFn: ({ id, fecha }: { id: string, fecha: string }) => updateSaleDate(id, fecha),
        onSuccess: () => {
            toast.success("Fecha actualizada exitosamente en todos los módulos.");
            qc.invalidateQueries({ queryKey: ['sales-history'] });
            qc.invalidateQueries({ queryKey: ['sales-stats-today'] });
            qc.invalidateQueries({ queryKey: ['inventario'] });
            setUpdateDateVenta(null);
            setNuevaFecha('');
        },
        onError: (err: any) => toast.error(err.message || 'Error al actualizar la fecha.')
    });
