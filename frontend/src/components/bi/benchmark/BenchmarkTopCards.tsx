...
export const BenchmarkTopCards: React.FC<Props> = ({
    ...
    return (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
            ...
            <div className="bg-rose-50/70 border border-rose-200/80 rounded-2xl p-3.5 shadow-2xs flex flex-col justify-between">
                <div className="flex items-center justify-between">
                    ...
                    <span className="bg-rose-100 text-rose-800 text-[9px] font-black px-2 py-0.5 rounded-md uppercase border border-rose-200">
                        CRÍTICO
                    </span>
                </div>
                <div className="my-1.5">
                    ...
                    <div className="text-[11px] font-bold text-slate-800 mt-0.5 leading-tight">
            ...
            {/* Card 2: P50 (Mediana) */}
            <div className="bg-blue-50/70 border border-blue-200/80 rounded-2xl p-3.5 shadow-2xs flex flex-col justify-between">
                <div className="flex items-center justify-between">
                    <span className="text-[11px] font-extrabold text-blue-800 flex items-center gap-1.5">
                        <span className="w-2 h-2 rounded-full bg-blue-500 inline-block shrink-0"></span> P50 (Mediana)
                    </span>
                    <span className="bg-blue-100 text-blue-800 text-[9px] font-black px-2 py-0.5 rounded-md uppercase border border-blue-200">
                        NORMAL
                    </span>
                </div>
                <div className="my-1.5">
            ...
            <div className="bg-emerald-50/70 border border-emerald-200/80 rounded-2xl p-3.5 shadow-2xs flex flex-col justify-between">
                <div className="flex items-center justify-between">
                    ...
                    <span className="bg-emerald-100 text-emerald-800 text-[9px] font-black px-2 py-0.5 rounded-md uppercase border border-emerald-200">
                        META
                    </span>
                </div>
                <div className="my-1.5">
            ...
            <div className="bg-white border-2 border-indigo-200 rounded-2xl p-3.5 shadow-xs flex flex-col justify-between space-y-1.5">
                <div className="flex items-center justify-between">
                    <span className="text-[11px] font-black text-indigo-900 flex items-center gap-1 uppercase tracking-wider">
                        <Zap className="w-3.5 h-3.5 text-purple-600 fill-purple-600" /> POSICIÓN ACTUAL
                    </span>
                    <span className={`text-[9px] font-black px-2 py-0.5 rounded-md border flex items-center gap-1 ${statusBg}`}>
                ...
                <div>
                    <div className="flex items-baseline gap-2">
                        <span className="text-xl font-black text-slate-900 font-mono tracking-tight">{formatValue(todaySales)}</span>
                    ...
                    <div className="flex items-center justify-between text-[11px] font-bold text-indigo-900 mt-0.5">
                        <span>Sobre el <strong>{percentilePositionPct.toFixed(0)}%</strong> del histórico</span>
                        <span className="font-mono text-slate-500 text-[10px]">{percentilePositionPct.toFixed(0)}%</span>
                    </div>
                ...
                <div className="grid grid-cols-3 gap-1 bg-slate-50 border border-slate-200/80 rounded-xl p-1.5 text-center text-[9px] pt-1">
                    <div>
                    ...
                    <div className="border-x border-slate-200 px-0.5">
                        <span className="text-slate-400 font-medium block">Promed. ({dayName.toLowerCase()}s):</span>
                        <strong className="text-slate-800 font-bold block font-mono text-[9.5px]">{formatValue(promedioEquivalente)}</strong>
                    </div>
                    <div>
                        <span className="text-slate-400 font-medium block">Variación:</span>
                        <strong className={`font-bold font-mono block text-[9.5px] ${variacionEquivalentePct >= 0 ? 'text-emerald-600' : 'text-rose-600'}`}>
