...
import { 
    Calendar, Loader2, 
    Clock, Zap, FileSpreadsheet, Activity
} from 'lucide-react';
...
const CustomDatePicker = ({ fechaRef, setFechaRef }: { fechaRef: string, setFechaRef: (f: string) => void }) => {
    ...
    return (
        <div className="relative flex items-center gap-2" ref={ref}>
            <div 
                className={`flex items-center gap-2 bg-gray-50 hover:bg-gray-100 border transition-colors rounded-xl px-3 py-1.5 shadow-xs cursor-pointer ${isOpen ? 'border-indigo-400 ring-2 ring-indigo-50' : 'border-gray-200'}`}
                        ...
                        {days.map((day, idx) => {
                            ...
                            return (
                                <button
                                    ...
                                    className={cn(
                                        "h-8 text-xs font-semibold rounded-lg flex flex-col items-center justify-center relative transition-all cursor-pointer",
                                        isSelected 
...
// ───────────────────────────────────────────────────────────────────────────────
// Componente Principal BIComparativasView (Copia Fiel y Exacta de Visión General)
// ───────────────────────────────────────────────────────────────────────────────
export interface BIComparativasViewProps {
    fechaRefProp?: string;
    sucursalProp?: string;
}

export const BIComparativasView: React.FC<BIComparativasViewProps> = ({
    fechaRefProp,
    sucursalProp,
}) => {
    const [fechaRef, setFechaRef] = useState<string>(fechaRefProp || getTodayDateString());
    ...
    const fetchData = useCallback(async (fecha: string, suc: string) => {
        ...
        try {
            ...
            const res: any = await getHourlyMultiyear(targetFecha, suc || undefined);
            ...
            console.error('BIComparativasView Multiyear error:', e);
            setIsError(true);
        } finally {
            setIsLoading(false);
        }
    ...
    return (
        <div className="bg-white/90 backdrop-blur-xl rounded-[2rem] p-6 shadow-sm border border-gray-100 flex flex-col h-full space-y-5 animate-in fade-in duration-300">

            {/* Título de Sección con Ícono */}
            <div className="flex items-center gap-2.5 pb-1">
                <Activity className="text-purple-600 stroke-[2.5]" size={22} />
                <h2 className="text-lg md:text-xl font-black text-gray-900 tracking-tight">
                    Comparativa Horaria Multi-Año
                </h2>
            </div>

            {/* Header de Filtros & Alineación Histórica */}
            <div className="w-full bg-slate-50/90 border border-slate-200/80 rounded-2xl px-5 py-3 shadow-xs flex flex-wrap items-center justify-between gap-4 min-h-[54px]">
                ...
                <div className="flex flex-wrap items-center gap-2 shrink-0">
                    <button
                        ...
                        className={`text-xs font-bold px-3 py-1.5 rounded-xl border transition-colors cursor-pointer shadow-2xs active:scale-95 ${
                    ...
                    <select
                        ...
                        className="px-3 py-1.5 bg-white border border-gray-200 hover:border-indigo-300 focus:border-indigo-500 rounded-xl font-bold text-xs text-gray-800 outline-none transition-all cursor-pointer appearance-none shadow-2xs"
                    >
                        {SUCURSALES.map(s => (
                    ...
                    <button
                        ...
                        className="flex items-center gap-1.5 text-xs font-black px-3.5 py-1.5 rounded-xl bg-white hover:bg-indigo-50 text-indigo-700 border border-indigo-200 shadow-2xs transition-all cursor-pointer"
            ...
            {/* Banner KPI Multianual de Rendimiento */}
            {meta && !isLoading && (() => {
                ...
                return (
                    <div className={cn("p-4 rounded-2xl border flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4 transition-all shadow-xs", bgStyle)}>
                        <div>
                        ...
                        <div>
                            ...
                            {totalVendidoAnio1 > 0 ? (
                                ...
                                <p className="text-xs font-bold text-slate-500 italic">Sin registros históricos para este día (2025)</p>
                            )}
                        </div>
                        ...
                        <div>
                            ...
                            {totalVendidoAnio2 > 0 ? (
                                ...
                                <p className="text-xs font-bold text-slate-500 italic">Sin registros históricos para este día (2024)</p>
                            )}
                        </div>
                        ...
                        <div className="text-left lg:text-right">
                            ...
                            <p className="text-xs font-black inline-flex items-center gap-1.5 px-3 py-1 bg-white/90 rounded-full shadow-xs">
            ...
            {/* Gráfico Recharts ComposedChart Exacto */}
            <div className="w-full h-[400px] min-h-[400px] pt-2">
                {isLoading ? (
                    ...
                    <div className="h-full flex items-center justify-center text-red-500 text-sm font-bold bg-red-50 rounded-2xl border border-red-100">
                        Error cargando datos de comparativa horaria.
                    </div>
                ) : (
                    <ResponsiveContainer width="100%" height="100%" minWidth={0}>
                        <ComposedChart data={renderedData} barGap={1} barCategoryGap="20%" margin={{ top: 25, right: 20, left: 35, bottom: 10 }}>
                            ...
                            {HORAS_OPERACION.includes(currentHourStr) && (
                                <ReferenceLine 
                                    x={currentHourStr} 
                                    stroke="#6366f1" 
                                    strokeWidth={1.5}
                    ...
                    {(() => {
                        ...
                        return (
                            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                                <div className="bg-white p-4 rounded-2xl border border-gray-100 shadow-xs flex items-center gap-3.5 h-full min-h-[90px]">
...
export default BIComparativasView;
